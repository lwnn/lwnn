...
int layer_cpu_q16_CONCAT_init(const nn_t* nn, const layer_t* layer)
{
	...
	if(0 == r)
	{
		...
		if(0 == r)
		{
			context = (layer_cpu_q16_concat_context_t*)layer->C->context;
			context->Q = Q;
		}
	}
...
int layer_cpu_q16_CONCAT_execute(const nn_t* nn, const layer_t* layer)
{
	...
	int16_t* pin;
	int16_t* pout = (int16_t*)context->out[0];
	size_t n_block;
	size_t in_stride;
	size_t out_stride;
	size_t i,j;
	...
	{	/* concat all input layers */
		input_context = (layer_cpu_q16_context_t*)(*input)->C->context;
		pin = (int16_t*)input_context->out[0];
		...
		for(i=0; i<n_block; i++)
		{
			memcpy(pout+i*out_stride, pin, in_stride*sizeof(int16_t));
			pin += in_stride;
		}
		pout += in_stride;
		input++;
