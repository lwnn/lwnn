...
int layer_cpu_s8_OUTPUT_init(const nn_t* nn, const layer_t* layer)
{
	int r = 0;
	int8_t* int8s;
	layer_cpu_s8_output_context_t* context;

	r = rte_cpu_create_layer_context(nn, layer,
				sizeof(layer_cpu_s8_output_context_t), 0);

	if(0 == r)
	{
		context = (layer_cpu_s8_output_context_t*)layer->C->context;
...
{
	int r = 0;
	layer_cpu_s8_output_context_t* context = (layer_cpu_s8_output_context_t*)layer->C->context;
